...
KPConv-Torch is a library with a *command-tine interface* (CLI). It works for training a neural network model on a points cloud dataset.
...
- `-t` or `--trained-model`: path to an already trained KPConv model log folder on the file system (mutually exclusive with the `-o` option);
- `-o` or `--output-folder`: path to a folder which will contain the new KPConv model log folder on the file system (mutually exclusive with the `-t` option).
...
- `-d` or `--datapath` (mandatory):
    - path to the folder of labeled points clouds data files used to train the model, which should contain two folders, generated by the preprocessing step:
        - `input_xxxx`;
        - `original_ply`;
- `-t` or `--trained-model`: path of the KPConv log folder on the file system, in which will be stored the trained model (mandatory);
...
- two files in a `calibration` sub-folder, which is located in the folder pointed by the `-d` option, containing the data used to train the model:
    - `batch_limits.pkl`: ;
    - `neighbors_limits.pkl`: ;
- three folders, in the folder pointed by the `-t` option, containing the trained model:
    - `potentials`: ;
